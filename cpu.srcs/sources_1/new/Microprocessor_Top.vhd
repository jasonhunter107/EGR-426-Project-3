...
signal clkout_1Hz, clkout_500Hz, clkout_1KHz : STD_LOGIC;
...
cd1: clk_divider port map (clkin => clk, reset => reset, clkout_1Hz => clkout_1Hz, clkout_500Hz => clkout_500Hz, clkout_1KHz=> clkout_1KHz);

M1: SevenSeg_Mux port map(clk => clkout_500Hz, reset => reset, LeftSeg => outport1, RightSeg => outport0, an => an, Seg_out => Seg_out);

p1: PWM port map (clk => clkout_1KHz, PWMin => PWMDutyCycle, outputSignal => Ledport15);
